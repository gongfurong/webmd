...
| **中栏** | 路径栏（见 §3.6）+ 正文/预览；底栏上一页/下一页（固定高度） |
...
| **多 sheet** | 底栏可**切换**已有表；**禁止**添加 / 删除 / 重命名表 |
| **文本** | **`mode: read`**：不可改单元格文字（防手机误弹键盘）；隐藏选区 hide-input 焦点 |
| **显示操作** | 可拖列宽/行高；工具栏最左自定义条：重载 · 全选 · 自动列宽 · 行高 · 缩放（数字，≥16px 防 iOS 整页放大） |
| **库工具栏** | 与自定义条同一行；装不下进「更多 …」；下拉/色板须盖住操作条文字 |
| **全屏** | 路径栏中栏全屏（含手机伪全屏） |
| **不写源** | 永不写回 content；重载丢弃会话显示调整 |
