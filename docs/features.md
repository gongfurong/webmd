...
| 区域 | 功能要点 |
|------|----------|
| **顶栏** | 窄屏打开文件抽屉 / 大纲抽屉；回首页；搜索；GitHub；明暗主题；固定/铺满版心 |
| **左栏** | 文件夹树（名序、单开/多开、类型分组）；滚动到顶/底；收起栏 |
| **中栏** | 路径面包屑、文件元信息、下载、URL 弹层；正文或媒体/PDF/表格预览；上一页/下一页 |
| **右栏** | 本页标题大纲，滚动联动高亮；可收起 |

### 2.1 窄屏 / 宽屏（视口，不是「手机/PC 设备类型」）

| 视口 | 行为 |
|------|------|
| **宽 ≤640** | 左右均为**上层抽屉**；默认内容宽度偏好「固定」（≈用满中栏） |
...
|------|------|
| **铺满 (fill)** | 正文/预览吃满中栏宽度 |
| **固定居中 (fixed)** | `max-width ≈ 视口 − 左右栏可拖最大宽 − 缝`；中栏更宽时**水平居中**；收侧栏不无限变宽 |

- **所有页面类型**（Markdown、PDF、媒体、表格、下载卡）在 fixed 下均遵守版心规则  
- 默认：宽屏偏 fill，窄屏偏 fixed（可记忆在 `localStorage`）  
- 横屏固定有**不低于竖屏宽度**的地板逻辑，避免被「屏−双栏」压得过窄  
...
站内文档链接默认**软导航**：主要替换中栏与大纲，减轻整页刷新；左侧树尽量保持展开/滚动状态。
...
| 项 | 说明 |
|----|------|
| **类型栏** | `CSV` / `XLSX` +「注意：可编辑•不写回」；密度/缩放/全选/重载/复制 CSV |
| **数据** | 浏览器 **fetch 原文件**（不依赖 `_Res_*` CSV） |
| **多 sheet** | x-spreadsheet 底栏 |
| **编辑** | 会话内可改；不写源；重载只重绘中栏 |
| **自动列宽/行高** | 工具栏最左（库无原生 API） |
| **全屏** | 路径栏「中栏全屏」（通用） |
| **内存** | 整表进内存 |

- shell：`scripts/lib/spreadsheet-preview.ts`  
