...
| **站内文件夹** | **不生成目录页**；见 §3.2.1 / §4.6 |

**下载**始终走 `/content/...` 原件，与预览页、文件夹链改写无关。

### 3.2 文件树工具

| 控件 | 作用 |
|------|------|
| **名序** | 文件名升序 / 降序（中文 locale + numeric）；**默认升序（名升）** |
| **单开 / 多开** | **默认单开**：只展开「进入链 ∪ focus 链」上的目录，其余折叠；**多开**可同时展开多个，但仍保证进入/focus 链打开。切回单开立即收拢 |
| **文上 / 夹上 / 混排** | 同层类型分组再名序；**默认文上**（文件在上、文件夹在下） |
| 滚到顶/底 | 树滚动 |
| 收起文件栏 | 宽屏收列 / 窄屏关抽屉 |

状态记忆：`localStorage`（`webmd-tree-sort` / `webmd-tree-group` / `webmd-tree-accordion` 等）。

### 3.2.1 文件夹三态（进入 / focus / 常规）

| 状态 | class | 含义 | 如何进入 |
|------|--------|------|----------|
| **常规** | （无） | 普通文件夹 | 默认 |
| **进入** | `is-on-path` | 当前**打开文件**的祖先目录 | 打开/切换文件页时由文件路径推导 |
| **focus** | `is-tree-focus` | 用户「点到」的目录 | 点树 summary、路径栏目录段、正文文件夹链；**打开文件时 focus = 该文件所在目录** |

| 操作 | 结果 |
|------|------|
| 打开/切换**文件** | 更新进入链；**focus → 文件父目录**；单开按规则展开/折叠 |
| 点**文件夹**（树 / 路径栏 / 正文 `./子目录/`） | **只改 focus**，不换中栏文档、不 404 |
| 路径栏**当前文件名** | focus → 所在目录，并滚到该文件 |
| 路径栏**根目录**图标 | 清 focus；进入仍跟当前文件（若有）；树滚顶 |

实现：`src/client.ts`（`applyTreeOpenState` / `treeFocusDirPath`）。**不**为文件夹单独 SSG 一页。

### 3.2.2 站内文件夹链接（Markdown）

| 写法 | 构建/渲染 | 点击 |
|------|-----------|------|
| `./子目录/` 或磁盘上存在的相对目录 | 改写为 `#tree-dir=&lt;content相对路径&gt;` | 与路径栏点文件夹相同 → 树 focus |
| `./某页.md` | 站内文档路由 | 软导航打开文件 |
| 站外 `http(s)://` | 不改 | 正常跳转 |

- **dev / build 只改写 href**，不生成额外 HTML 页面。  
- 显式链到 `README.md` 仍当**文件**打开。  
- 改写：`scripts/lib/markdown.ts` → `rewriteRelativeToContent`。
...
站内**文件**文档链接默认**软导航**（替换中栏 / 路径栏 / 大纲 / 分页；树按 §3.2.1 同步进入+focus）：
...
| **文件夹链** | `#tree-dir=` / 可识别的目录路径 → **不**进 soft-nav，只 focus 树 |
...
| 控件 | 行为 |
|------|------|
| **根目录图标**（磁盘） | **不进主页**；清 focus；展开文件栏并滚到树顶 |
| **目录段** | 点击 → **focus** 该文件夹并展开/滚动（不换页） |
| **文件名** | 点击 → focus 所在目录并滚到当前文件 |
| **info** | 打开**文件信息**弹层（大小·格式一行；可读/转义 URL；三复制按钮一排） |
| **版心** | 铺满 / 固定居中切换 |
| **中栏全屏** | 桌面：Fullscreen API；**手机**：CSS 伪全屏（iOS 不支持任意元素全屏） |
| **下载** | 全类型可用；链到 **`/content/` 原件**（见 §6） |
| **滚顶/底** | 中栏滚动 |
...
### 4.6 相对链接改写（构建 / dev 渲染期）

| 相对目标 | 改写结果 |
|----------|----------|
| `.md` / `.mdx` | 站内预览路径（可匹配 `/pages/...`） |
| **文件夹**（尾 `/` 或磁盘上是目录） | `#tree-dir=` + content 相对路径（§3.2.2） |
| 其它文件 | `/content/...` 原件 URL |

**验收**：README 索引写 `./子主题/` → 点击左侧树定位到该夹，中栏 URL 不变；下载按钮仍下原 `.md` 等文件。
